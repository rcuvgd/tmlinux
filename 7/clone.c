...
static char clone1_stack[8192], clone2_stack[8192], clone3_stack[8192];
...
int main(void) 
{
	...
	printf("clone.c main in!\n");

	signal(SIGCHLD, child_handler);

	if ((clone1 = clone(clone_main, clone1_stack+sizeof(clone1_stack), 0, (void*)11)) == -1) {
		printf("Clone 1 failed");
	...
	if ((clone2 = clone(clone_main, clone2_stack+sizeof(clone2_stack), 0, (void*)22)) == -1) {
		perror("Clone 2 failed");
		exit(-2);
	}
	if ((clone3 = clone(clone_main, clone3_stack+sizeof(clone3_stack), 0, (void*)33)) == -1) {
