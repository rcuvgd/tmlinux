...
ssize_t __getdents (int fd, char *buf, size_t nbytes)
{
    ...
    buf += ret;
    while ((unsigned char*) dp64 < (unsigned char*) buf) {
	dp = (struct dirent *) dp64;
	dp->d_ino = dp64->d_ino;
	dp->d_off = dp64->d_off;
	dp->d_reclen = dp64->d_reclen;
	dp->d_type = dp64->d_type;
	__memmove (dp->d_name, dp64->d_name, dp->d_reclen - offsetof (struct dirent64, d_name));
	__memmove (dp64, dp, dp->d_reclen);
#ifndef __TCS__
	dp64 = ((unsigned char*) dp64) + dp->d_reclen;
#else
	dp64 = (struct dirent64*)(((unsigned char*) dp64) + dp->d_reclen);
#endif 
    }
