...
int main(int ac, char **av)
{
	...
	for (lc = 0; TEST_LOOPING(lc); lc++) {
		...
		for (i=0; i<TST_TOTAL; i++) {
			 ...
			 */
			errno = 0;
			addr = shmat(*(tc.shmid), (unsigned char*)base_addr + tc.offset,
				   tc.flags);
			TEST_ERRNO = errno;
...
check_functionality(int i)
{
	...
	case 1:
		...
		*shared = CASE1;
		orig_add = (unsigned char*)addr + ((unsigned long)tc.offset%SHMLBA);
		if (orig_add != (unsigned char*)base_addr + tc.offset) {
